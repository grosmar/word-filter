...
class WordFilterView extends View<{model:WordFilterModel}>
{
	...
	function render() '
	<div>
		Filter: <input oninput={model.filter=event.target.value} value={model.filter}  style="width: 200p;"/> 
		Min: <input id="min" oninput={model.min=Std.parseInt(event.target.value) <= model.max ? Std.parseInt(event.target.value) : model.max} value={Std.string(model.min)} />
		Max: <input id="max" oninput={model.max=Std.parseInt(event.target.value) >= model.min ? Std.parseInt(event.target.value) : model.min} value={Std.string(model.max)} />
